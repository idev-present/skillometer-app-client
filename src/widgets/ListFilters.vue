    ...
    <div v-if="isShowFilters" class="p-4 pt-0 lg:p-0">
      ...
      <div class="w-full lg:mt-4">
        <Combobox
            ...
            @update:modelValue="queryQualification = ''"
        >
          ...
          <div class="relative mt-0.5">
            <ComboboxButton class="relative w-full">
              <ComboboxInput class="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-8 text-gray-900 shadow-sm ring-1 outline-0 ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6"
                             ...
                             @change="queryQualification = $event.target.value"
                             @blur="queryQualification = ''"
              />
              <div class="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none">
                <ChevronUpDownIcon class="h-5 w-5 text-gray-400" aria-hidden="true" />
              </div>
            </ComboboxButton>
            <ComboboxOptions v-if="filteredQualification?.length > 0" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              <ComboboxOption v-for="(item, index) in filteredQualification" :key="index" :value="item" as="template" v-slot="{ active, selected }">
...
const queryQualification = ref('')
const queryDivision = ref('')

const filteredQualification = computed(() => {
  return queryQualification.value === ''
      ? qualificationList.value
      : qualificationList?.value?.filter((item) => {
        return item.name.toLowerCase().includes(queryQualification.value.toLowerCase())
  }) || []
})
